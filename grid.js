...
function loc2latlon(locator) {
  ...
  var lon =
    ...
    180;
  var lat =
    loca[1] * 10 + loca[3] + loca[5] / 24 + loca[7] / 240 + loca[9] / 5760 - 90;
        ...
        for (var j = 0; j < 24; j++) {
          ...
          gridLayer.addLayer(
            L.rectangle(
              [
                [slat, slon],
                [slat + subLatStep, slon + subLonStep],
              ],
              gridStyle,
            ),
          );
          ...
          if (showLabels) {
            labelLayer.addLayer(
              getLabel(slon + 0.8 / 24, slat + 1 / 48, 3, zoom),
            );
          }
        }
      }
...
async function map_init() {
  ...
  mymap = L.map("mapid", { minZoom: 3, maxZoom: 14 }).setView(
    [mylat, mylon],
    zoomLevel,
  );
  ...
  mymap.on("zoomend", () => {
    ...
    const nextAllowed = allowedZoomLevels
      .filter((zoom) =>
        direction === 1 ? zoom > currentZoom : zoom < currentZoom,
      )
